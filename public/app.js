...
function newTaskPage() {
  layout("new", `<section class="page-title"><div class="eyebrow">Phase 2 · 单作品闭环</div><h1>采集一条作品</h1><p class="lead">输入真实作品链接，工具会调用本机 Edge 打开页面并保存一张真实截图。能读取到的公开数据才会展示，读取不到的字段保持为空。</p></section><section class="card form-card"><form id="task-form"><div class="field"><label for="task-name">任务名称</label><input id="task-name" name="name" placeholder="例如：单条作品验收" required maxlength="80" /></div><div class="field"><label for="source">作品链接</label><input id="source" name="source" type="url" placeholder="粘贴 TikTok / Instagram / Facebook / 抖音 / 快手作品链接" required /><small>请提前在浏览器中登录需要采集的平台。工具不会要求或保存账号密码。</small></div><div class="field"><label for="platform">平台</label><select id="platform" name="platform"><option value="tiktok">TikTok</option><option value="instagram">Instagram</option><option value="facebook">Facebook</option><option value="douyin">抖音</option><option value="kuaishou">快手</option></select></div><div class="form-actions"><button class="primary-button" type="submit">开始真实采集</button><button class="secondary-button" type="button" data-page="home">取消</button></div></form><div class="notice">当前阶段只支持单条作品链接。采集失败会显示真实失败原因，不会用示例截图或固定数据代替。</div></section><div id="capture-result"></div>`);
